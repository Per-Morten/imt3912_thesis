...
\end{gnuplot}
\caption{Numerous Unique Components, 200 unique components, time}
...
\todo{\paragraph{Too Many Unknown Data Access}?}
...
the whole scheduling of the function. We also argue that the branching on the "mock" implementation of virtual is quickly
speculatively loaded and executed, as the functions are called in a cache friendly manner.
...
This should not be surprising considering that the problem within the test case lends itself well to multi-threading.
