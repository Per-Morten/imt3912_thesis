...
\todo{Add small note explaining that the clearing of the allocator cannot be done concurrently}
...
\phantomsection
\paragraph{No ABA Problem}
\label{par:detailed_lock_free_allocator_aba}
Having a single clear function, means that collections using the allocator does not need to worry about being given a reused
piece of memory before having been cleared, which helps avoiding the ABA problem. \todo{Write shortly about the aba problem}
